...
$ is distributive over #, which means that x $ (a # b) = (x $ a) # (x $ b)
*/
...
template <class T> struct lazy_segment_tree {
    ...
    lazy_segment_tree(int n, function<T(const T&, const T&, int)> modify_operation, function<T(const T&, const T&)> query_operation, 
    ...
    void propagate(int p, int l, int r) {
        if (l == r) return;
        int m = l + r >> 1;
        apply_modify(op[p << 1], op[p], 1);
        apply_modify(op[p << 1 | 1], op[p], 1);
        apply_modify(tree[p << 1], op[p], r - m);
        apply_modify(tree[p << 1 | 1], op[p], m - l + 1);
        op[p] = modify_base;
    }
    ...
    T query(int L, int R, int p, int l, int r) {
        ...
        int m = l + r >> 1;
        return query_op(query(L, R, p << 1, l, m), query(L, R, p << 1 | 1, m + 1, r));
    }
