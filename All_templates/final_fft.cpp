...
template <class T> struct poly {
	...
	const static int MX = 56e4 + 9;
	const static int LOG = 21;
	cd pre_w[LOG][MX], pre_w_inv[LOG][MX];
	bool is_w_const;
	...
	void pre() {
		...
		for (int lg = 1; lg < LOG; lg++) {
			int len = (1 << lg);
			for (int j = 0; j < MX; j++) {
				double ang = j * 2 * PI / len;
				double inv_ang = -j * 2 * PI / len;
				pre_w[lg][j] = cd(cos(ang), sin(ang));
				pre_w_inv[lg][j] = cd(cos(inv_ang), sin(inv_ang));
			}
		}
	}
		...
		for (int len = 2, lg = 1; len <= n; len <<= 1, lg++) 
			for (int i = 0; i < n; i += len) 
				for (int j = 0; j < len / 2; j++) {
					cd w = (invert ? pre_w_inv[lg][j] : pre_w[lg][j]);
